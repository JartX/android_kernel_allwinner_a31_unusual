  ...
  { 
      ...
      if((dev->isp_gen_set_pt->isp_nigth_mode_flag == 1 ||
        ...
        dev->isp_3a_result_pt->exp_line_num > dev->isp_gen_set_pt->stat.intg_max ||
            dev->isp_gen_set_pt->isp_ini_cfg.adaptive_frame_rate == 1)&&
            dev->isp_gen_set_pt->stat.pic_size.height>1080)
      {
          if(dev->isp_gen_set_pt->isp_ini_cfg.high_quality_mode_en == 1 ||
            dev->isp_gen_set_pt->isp_ini_cfg.adaptive_frame_rate == 1)
          {
            val = dev->isp_3a_result_pt->sensor_exp_gain;
            
            if(v4l2_subdev_call(dev->sd,core,ioctl,ISP_SET_EXP_GAIN,&val) != 0)
            {
              vfe_err("set sensor exp gain error!\n");
            } else {
              dev->ctrl_para.prev_exp_gain = dev->isp_3a_result_pt->sensor_exp_gain;
            }
          }
          
          //printk("open flash when nigth mode\n");
            #ifdef _FLASH_FUNC_
		    if(((dev->isp_gen_set_pt->exp_settings.flash_mode != FLASH_MODE_OFF &&
                dev->isp_gen_set_pt->stat.curr_ang_gain > dev->isp_gen_set_pt->stat.gain_max*3/4) ||
                dev->isp_gen_set_pt->exp_settings.flash_mode == FLASH_MODE_ON)&&
                dev->isp_gen_set_pt->take_pic_start_cnt == 1)
		    {
		      printk("open flash when nigth mode\n");
			...
			#ifdef _FLASH_FUNC_
            if(dev->isp_gen_set_pt->exp_settings.flash_mode!=FLASH_MODE_OFF && 
                dev->isp_gen_set_pt->take_pic_start_cnt == 4)
            {
                 printk("close flash when nigth mode\n");
      ...
      else
      {
        ...
        if(dev->ctrl_para.prev_exp_line != dev->isp_3a_result_pt->exp_line_num || 
          ...
          dev->isp_gen_set_pt->isp_ini_cfg.isp_test_mode != 0)
          {
            if(dev->isp_3a_result_pt->exp_line_num > 0)
            {
	  ...
	  dev->ctrl_para.af_status_next = common_af_status_to_v4l2(dev->isp_3a_result_pt->af_status);
	  if(dev->ctrl_para.af_status_next != dev->ctrl_para.af_status_cur)
	  {
	    //TODO
	    //trigger waite queue
	  }
	  dev->ctrl_para.af_status_cur = dev->ctrl_para.af_status_next;
...
static int internal_s_input(struct vfe_dev *dev, unsigned int i)
{
  ...
  if(dev->sd_act!=NULL)
  {
    ...
    vcm_para.active_min = dev->isp_gen_set_pt->isp_ini_cfg.vcm_min_code;
    vcm_para.active_max = dev->isp_gen_set_pt->isp_ini_cfg.vcm_max_code;
    vfe_dbg(0,"min/max=%d/%d \n", dev->isp_gen_set_pt->isp_ini_cfg.vcm_min_code,
                                  dev->isp_gen_set_pt->isp_ini_cfg.vcm_max_code);
  ...
  if(dev->is_isp_used && dev->is_bayer_raw) {
    switch (ctrl->id) {
      ...
      case V4L2_CID_AUTOGAIN:
        ...
        break;
      case V4L2_CID_GAIN: 
        return CLIP(dev->isp_3a_result_pt->exp_analog_gain,0,255)+(dev->isp_gen_set_pt->isp_ini_cfg.color_denoise_level << 8);
      case V4L2_CID_HFLIP:
        ctrl->value = dev->ctrl_para.hflip; 
        break;
      case V4L2_CID_VFLIP:
      ...
      case V4L2_CID_EXPOSURE_AUTO:
        ...
        break;
      case V4L2_CID_EXPOSURE_ABSOLUTE:
        ctrl->value = dev->ctrl_para.exp_abs;
        break;
      case V4L2_CID_EXPOSURE_AUTO_PRIORITY:
...
static int vfe_resource_request(struct platform_device *pdev ,struct vfe_dev *dev)
{
  ...
  addr_handle = sunxi_mem_alloc(ISP_LOAD_REG_SIZE);
  if(addr_handle) {
    dev->regs.isp_load_regs_paddr = (void*)addr_handle;
    dev->regs.isp_load_regs_dma_addr = (void*)(addr_handle - CPU_DRAM_PADDR_ORG);
    dev->regs.isp_load_regs = ioremap(addr_handle, ISP_LOAD_REG_SIZE);
  ...
  addr_handle = sunxi_mem_alloc(ISP_SAVED_REG_SIZE);
  if(addr_handle) {
    dev->regs.isp_saved_regs_paddr = (void*)addr_handle;
    dev->regs.isp_saved_regs_dma_addr = (void*)(addr_handle - CPU_DRAM_PADDR_ORG);
    dev->regs.isp_saved_regs = ioremap(addr_handle, ISP_SAVED_REG_SIZE);
...
static void probe_work_handle(struct work_struct *work)
{
  ...
  { 
    ...
    if (i2c_adap == NULL) {
      vfe_err("request i2c adapter failed,input_num = %d\n",input_num);
      ret = -EFAULT;
      goto probe_hdl_free_dev;
    }
...
reg_sd:         
  ...
  {
  i2c_adap_act = i2c_get_adapter(dev->ccm_cfg[input_num]->twi_id);//must use the same twi_channel with sensor

  if (i2c_adap_act == NULL) {
    vfe_err("request act i2c adapter failed,input_num = %d\n",input_num);
    ret = -EINVAL;
  ...
  dev->ccm_cfg[input_num]->sd_act = v4l2_i2c_new_subdev_board(&dev->v4l2_dev,
                    i2c_adap_act,
                    //dev_sensor[input_num].type,//linux-3.0
                    &dev->dev_act[input_num],
                    NULL);
//reg_sd_act:         
  ...
  dev->ccm_cfg[input_num]->act_ctrl = 
    (struct actuator_ctrl_t *)container_of(dev->ccm_cfg[input_num]->sd_act,
    struct actuator_ctrl_t,
    sdev);
  //printk("dev->ccm_cfg[input_num]->act_ctrl=%x\n",(unsigned int )dev->ccm_cfg[input_num]->act_ctrl);
  }     
...
//    dev->ccm_cfg[input_num]->ccm_info.stby_mode = dev->ccm_cfg[input_num]->ccm_info.stby_mode;
    ...
    /*power issue*/
    dev->ccm_cfg[input_num]->power.iovdd = NULL;
    dev->ccm_cfg[input_num]->power.avdd = NULL;
    dev->ccm_cfg[input_num]->power.dvdd = NULL;
    dev->ccm_cfg[input_num]->power.afvdd = NULL;
    ...
    if(strcmp(dev->ccm_cfg[input_num]->afvdd_str,"")) {
      dev->ccm_cfg[input_num]->power.afvdd = regulator_get(NULL, dev->ccm_cfg[input_num]->afvdd_str);
      if (dev->ccm_cfg[input_num]->power.afvdd == NULL) {
        vfe_err("get regulator csi_afvdd error!input_num = %d\n",input_num);
        goto probe_hdl_unreg_dev;
      }
    }
  }
...
static struct resource vfe_vip0_resource[] = {
  ...
  [1] = {
    .start  = MIPI_CSI_REGS_BASE,
    .end    = MIPI_CSI_REGS_BASE + MIPI_CSI_REG_SIZE - 1,
    .flags  = IORESOURCE_MEM,
  },
  [2] = {
    .start  = MIPI_DPHY_REGS_BASE,
    .end    = MIPI_DPHY_REGS_BASE + MIPI_DPHY_REG_SIZE - 1,
    .flags  = IORESOURCE_MEM,
  },
  [3] = {
    .start  = ISP_REGS_BASE,
