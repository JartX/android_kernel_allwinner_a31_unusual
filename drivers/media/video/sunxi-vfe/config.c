...
int fetch_isp_cfg(struct vfe_dev *dev, struct cfg_section *cfg_section)
{
  ...
  if(dev->isp_gen_set[isp_id].isp_ini_cfg.isp_test_mode != 0)
  {
	...
	if (CFG_ITEM_VALUE_TYPE_INT != type)
	{
	  vfe_warn("fetch isp_dbg_level from camera.ini failed,apply default value!\n");
	...
	if (CFG_ITEM_VALUE_TYPE_INT != type)
	{
	  vfe_warn("fetch isp_focus_len from camera.ini failed,apply default value!\n");
	...
	if (CFG_ITEM_VALUE_TYPE_INT != type)
	{
	  vfe_warn("fetch isp_exp_line from camera.ini failed,apply default value!\n");
	...
	if (CFG_ITEM_VALUE_TYPE_INT != type)
	{
	  vfe_warn("fetch isp_gain from camera.ini failed,apply default value!\n");
    ...
    if (CFG_ITEM_VALUE_TYPE_INT != type)
    {
      vfe_warn("fetch ae_en from camera.ini failed,apply default value!\n");
    ...
    if (CFG_ITEM_VALUE_TYPE_INT != type)
    {
      vfe_warn("fetch af_en from camera.ini failed,apply default value!\n");
    ...
    if (CFG_ITEM_VALUE_TYPE_INT != type)
    {
      vfe_warn("fetch awb_en from camera.ini failed,apply default value!\n");
    ...
    if (CFG_ITEM_VALUE_TYPE_INT != type)
    {
      vfe_warn("fetch drc_en from camera.ini failed,apply default value!\n");
      dev->isp_gen_set[isp_id].isp_ini_cfg.drc_en = 1;
    }
    else
    {
  ...
  /* fetch front_camera enable! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","front_camera",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch front_camera from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.front_camera = 0;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.front_camera =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.front_camera = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.front_camera);
  }
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch sprite_en from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch satur_en from camera.ini failed,apply default value!\n");
	dev->isp_gen_set[isp_id].isp_ini_cfg.satur_en = 1;
  }
  else
  {
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch pri_contrast_en from camera.ini failed,apply default value!\n");
	dev->isp_gen_set[isp_id].isp_ini_cfg.pri_contrast_en = 1;
  }
  else
  {
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch lsc_en from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch high_quality_mode_en from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch defog_en from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch vcm_min_code from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch vcm_max_code from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch pri_contrast from camera.ini failed,apply default value!\n");
  ...
  /* fetch flicker_type! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","flicker_type",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch flicker_type from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.flicker_type = 1;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.flicker_type =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.flicker_type = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.flicker_type);
  }

  /* fetch adaptive_frame_rate! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","adaptive_frame_rate",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch adaptive_frame_rate from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.adaptive_frame_rate = 0;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.adaptive_frame_rate =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.adaptive_frame_rate = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.adaptive_frame_rate);
  }
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch denoise_level from camera.ini failed,apply default value!\n");
  ...
  /* fetch color_denoise_level! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","color_denoise_level",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch color_denoise_level from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.color_denoise_level = 0;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.color_denoise_level =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.color_denoise_level = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.color_denoise_level);
  }
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch sharpness_level from camera.ini failed,apply default value!\n");
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch defog_value from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.defog_value = 128;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.defog_value =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.defog_value = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.defog_value);
  }

  /* fetch brightness_tune! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","brightness_tune",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch brightness_tune from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.brightness_tune = 0;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.brightness_tune =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.brightness_tune = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.brightness_tune);
  }
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch gain_delay_frame from camera.ini failed,apply default value!\n");
  ...
  else
  {
	...
	vfe_dbg(0,"isp_ini_cfg.gain_delay_frame = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.gain_delay_frame);
  }


  /* fetch r_gain_2900k! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","r_gain_2900k",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_dbg(0,"fetch r_gain_2900k from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.r_gain_2900k = 385;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.r_gain_2900k =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.r_gain_2900k = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.r_gain_2900k);
  }
  /* fetch b_gain_2900k! */
  type = cfg_get_one_subkey(cfg_section,"isp_func","b_gain_2900k",&subkey);
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_dbg(0,"fetch b_gain_2900k from camera.ini failed,apply default value!\n");
    dev->isp_gen_set[isp_id].isp_ini_cfg.b_gain_2900k = 140;
  }
  else
  {
    dev->isp_gen_set[isp_id].isp_ini_cfg.b_gain_2900k =subkey.value->val;
	vfe_dbg(0,"isp_ini_cfg.b_gain_2900k = %d\n",dev->isp_gen_set[isp_id].isp_ini_cfg.b_gain_2900k);
  }
  ...
  {
    vfe_warn("fetch gamma_tbl from camera.ini failed!\n");
  }
  else
  {
  ...
  if(dev->isp_gen_set[isp_id].isp_ini_cfg.lsc_en == 1)
  {
	  ...
	  if (CFG_ITEM_VALUE_TYPE_STR != type)
	  {
		vfe_warn("fetch lsc_tbl from camera.ini failed!\n");
	  }
	  else
	  {
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch lsc_center_x from camera.ini failed!\n");
	dev->isp_gen_set[isp_id].isp_ini_cfg.lsc_center[0] = 2048;
  }
  else
  {
  ...
  if (CFG_ITEM_VALUE_TYPE_INT != type)
  {
    vfe_warn("fetch lsc_center_y from camera.ini failed!\n");

	dev->isp_gen_set[isp_id].isp_ini_cfg.lsc_center[1] = 2048;
  }
  else
  {
  ...
  for(j = 0;j<6;j++)
  {
	sprintf(sub_name, "csc_coeff_%d", j);
	  type = cfg_get_one_subkey(cfg_section,"isp_csc",sub_name,&subkey);
	  if (CFG_ITEM_VALUE_TYPE_INT != type)
	  {
		vfe_warn("fetch csc_coeff_%d from camera.ini failed,apply default value!\n",j);
		dev->isp_gen_set[isp_id].isp_ini_cfg.csc_coeff[j]=1024;
	  }
	  else
	  {
		dev->isp_gen_set[isp_id].isp_ini_cfg.csc_coeff[j]=subkey.value->val;
		vfe_dbg(0,"isp_ini_cfg.isp_ini_cfg.csc_coeff_[%d] = %d\n",j,dev->isp_gen_set[isp_id].isp_ini_cfg.csc_coeff[j]);
	  }
  }
  ...
  for(j = 0;j<8;j++)
  {
	sprintf(sub_name, "gain_offset_%d", j);
	  ...
	  if (CFG_ITEM_VALUE_TYPE_INT != type)
	  {
		vfe_warn("fetch gain_offset_%d from camera.ini failed,apply default value!\n",j);
		if(j<4)
		{
   ...
   for(j = 0;j<9;j++)
   {
	 sprintf(sub_name, "matrix_%d", j);
	   ...
	   if (CFG_ITEM_VALUE_TYPE_INT != type)
	   {
		 vfe_warn("fetch matrix_%d from camera.ini failed!\n",j);
	   }
	   else
	   {
   ...
   for(j = 0;j<3;j++)
   {
	 sprintf(sub_name, "offset_%d", j);
	   ...
	   if (CFG_ITEM_VALUE_TYPE_INT != type)
	   {
		 vfe_warn("fetch offset_%d from camera.ini failed!\n",j);
	   }
	   else
	   {
		 offset[j]=subkey.value->val;

		 vfe_dbg(0,"offset[%d] = %d\n",j,offset[j]);
	   }
   }

   /* fetch ISP color matrix_2! */
   matrix = (short*)dev->isp_gen_set[isp_id].isp_ini_cfg.color_matrix_2.matrix;
   offset = (short*)dev->isp_gen_set[isp_id].isp_ini_cfg.color_matrix_2.offset;
   for(j = 0;j<9;j++)
   {
	 sprintf(sub_name, "matrix_2_%d", j);
	   type = cfg_get_one_subkey(cfg_section,"isp_color_matrix_2",sub_name,&subkey);
	   if (CFG_ITEM_VALUE_TYPE_INT != type)
	   {
		 vfe_dbg(0,"fetch matrix_%d from camera.ini failed!\n",j);
	   }
	   else
	   {
		 matrix[j]=subkey.value->val;
   ...
   for(j = 0;j<3;j++)
   {
	 sprintf(sub_name, "offset_2_%d", j);
	   type = cfg_get_one_subkey(cfg_section,"isp_color_matrix_2",sub_name,&subkey);
	   if (CFG_ITEM_VALUE_TYPE_INT != type)
	   {
		 vfe_dbg(0,"fetch offset_2_%d from camera.ini failed!\n",j);
	   }
	   else
	   {
		 offset[j]=subkey.value->val;
