...
static __s32 camera_detect_gc0307(__u32 list_index, struct i2c_adapter *i2c_adap)
{
	...
	reg_num[0] = 0xf0; //Jeff change 0xfe to 0x f0
	value[0] = 0x00; //PAGE 0x00
	ret = camera_i2c_write(i2c_adap, reg_num, value, i2c_addr, addr_step, data_step);
	if (ret < 0) {
		return ret;
...
{   
	__camera_gpio_set_t *stby_pin_gpio = camera_info->stby_pin;
	list_print("Jeff: 0328stby port(%d),port_num(%d),mul_sel(%d),pull(%d),data(%d),gpio(%d) \n",
		stby_pin_gpio->port, stby_pin_gpio->port_num,stby_pin_gpio->mul_sel,
		stby_pin_gpio->pull,  stby_pin_gpio->data, stby_pin_gpio->gpio);

	__camera_gpio_set_t *reset_pin_gpio = camera_info->reset_pin;
	list_print("Jeff: 0328reset port(%d),port_num(%d),mul_sel(%d),pull(%d),data(%d),gpio(%d) \n",
		reset_pin_gpio->port, reset_pin_gpio->port_num,reset_pin_gpio->mul_sel,
		reset_pin_gpio->pull,  reset_pin_gpio->data, reset_pin_gpio->gpio);
...
static __s32 camera_detect_gc0328(__u32 list_index, struct i2c_adapter *i2c_adap)
{
	...
	if (ret < 0)
	{
		list_print("sensor_write err at sensor_detect!\n");
		return ret;
	}
	...
	if (ret < 0) 
	{
		list_print("sensor_read err at sensor_detect!\n");
		return ret;
	}
...
static void camera_pwr_on_siv121d(__u32 list_index, __camera_info_t *camera_info)
{
    //power on reset
	camera_gpio_set_status(camera_info->stby_pin,1);//set the gpio to output
	camera_gpio_set_status(camera_info->reset_pin,1);//set the gpio to output
...
static void camera_pwr_off_siv121d(__u32 list_index, __camera_info_t *camera_info)
{
    //power supply off
	if(camera_info->iovdd) {
		regulator_disable(camera_info->iovdd);
...
static __s32 camera_detect_siv121d(__u32 list_index, struct i2c_adapter *i2c_adap)
{
	...
	if(regs.value[0] != 0xde)
		return -ENODEV;

    list_print("detect siv121d success!!\n");
...
__camera_list_t camera_list[MAX_CAMERA_LIST_ITEM] = {
    ...
    CAMERA_LIST_ITEM_INIT(gc0328,        1, 1, 0x42, 1, 0, 0, 1, 1, 0), //add Jeff
    ...
    CAMERA_LIST_ITEM_INIT(siv121d,       1, 1, 0x66, 1, 0, 0, 1, 1, 0),
};
