...
static int sensor_s_exp_gain(struct v4l2_subdev *sd, unsigned int exp_gain)
{
    ...
    if(exp_val<=((2000*16+8)>>4))
    {
        sensor_write(sd, 0x0340, 0x07);//coarse integration time
        sensor_write(sd, 0x0341, 0xd0);
    ...
    {
        sensor_write(sd, 0x0340, 0x0f);//coarse integration time
        sensor_write(sd, 0x0341, 0xa0);
