...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
  ...
  sensor_read(sd,0x04,&tmp1);
  sensor_read(sd,0x03,&tmp2);
  intg = tmp2*256 + tmp1;
  //printk("gc2235 sensor exposure line is %d\n",exp_val);
 // printk("sensor read out exp is %d\n",intg);
	info->exp = exp_val;
	return 0;
}
...
{
	unsigned char gain1 = 0,gain2=0,tmp;
	int gain;
	struct sensor_info *info = to_state(sd);
	...
	else if (gain_val>=256 && gain_val<=512)
		{
			tmp = gain_val /256;
			sensor_write(sd,0xb0,tmp*64);
			sensor_write(sd,0xb1,0xff);
			sensor_read(sd,0xb0,&gain1);
			sensor_read(sd,0xb1,&gain2);
			gain = (unsigned char)((gain1/64)*(gain2/64));
	  }
...
static int sensor_s_exp_gain(struct v4l2_subdev *sd, unsigned int exp_gain)
{
  ...
  if(exp_val > 1234*16)
  {
	sensor_write(sd,0x07,0x04); //Now frame rate is 10fps
		sensor_write(sd,0x08,0x84);
  ...
  {
	sensor_write(sd,0x07,0x00);//Now frame rate is 19fps
    sensor_write(sd,0x08,0x1a);
    sensor_write(sd,0xb2, 0x1);
    sensor_s_exp(sd,exp_val);
    sensor_s_gain(sd,gain_val);
  }
  ...
  if(exp_val > 1288*16)
  {
	sensor_write(sd,0x06,0x28); //Now frame rate is 12fps
	sensor_write(sd,0x07,0x05);
    sensor_write(sd,0x08,0xbf);
    sensor_write(sd,0xb2, 0x1);
    sensor_s_exp(sd,exp_val);
    sensor_s_gain(sd,gain_val);
  }
  ...
  {
    sensor_write(sd,0x06,0x28);//Now frame rate is 25fps
		sensor_write(sd,0x07,0x00);
		sensor_write(sd,0x08,0x38);
		sensor_write(sd,0xb2,0x1);
    sensor_s_exp(sd,exp_val);
    sensor_s_gain(sd,gain_val);
  }
