	...
	if(b_en)
	{
		...
		/*        
			...
			*/
	        ...
	        hdl = OSAL_GPIO_Request(gpio_info, 1);
	        OSAL_GPIO_Release(hdl, 2);
	        //udelay(200); //removed by lyp@20140522 for LG panel
			Disp_lcdc_pin_cfg(sel, DISP_OUTPUT_TYPE_LCD, 1); //add by lyp @20140522
        ...
        //    msleep(30);
			/* add by lyp@20140522 start */
			else if(gpanel_info[sel].lcd_if == LCD_IF_EXT_DSI)
	        {
	            LCD_POWER_ELDO3_EN(sel, 1, 12);
	            msleep(10);
				}
	        /* add by lyp@20140522 end */
        }
