        ...
        if(!psDispcData->show_black[disp])
        {
            ...
            for(i=0; i<4; i++)
            {
                ...
                if(i < layer_num)
                {
                    ...
                    else if(layer_info.fb.mode == DISP_MOD_NON_MB_UV_COMBINED)
                    {
                        if(layer_info.fb.format == DISP_FORMAT_YUV420)
                        {
                            layer_info.fb.addr[1] = PAGE_ALIGN(layer_info.fb.addr[0] + layer_info.fb.size.width * layer_info.fb.size.height);
                        }
                    }
