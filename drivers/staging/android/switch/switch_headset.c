...
static struct gpio_switch_data *switch_data1;
...
static void earphone_switch_timer_poll(unsigned long data)
{
	...
	if (((tmp >= 0xb) && (switch_data->mode != FOUR_HEADPHONE_PLUGIN) && (switch_data->state != 2) && (switch_data->reset_flag == 0))&&(switch_data->mode!=HOOK_CHCCK_DOWN)) {
		...
		if ((switch_data->check_three_count > CIRCLE_COUNT) && (switch_data->state != 2)) {
			...
			hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_DIRQ, 0x0);					/*23*/
		}
		...
		if ((tmp >= 0xe)&&(switch_data->reset_flag==0)) {
			SWITCH_DBG("H_SCH\n");
...
static irqreturn_t audio_hmic_irq(int irq, void *dev_id)
{
	...
	/*检测四节耳机拔出过程中，会跳到三节耳机状态，期间可能有一个耳机拔出状态，此时需重新检测，防止hook键误判*/
	...
	if (!headphone_direct_used) {
		if ((switch_data->state == -1)||(tmp >= 1)) {
			...
			if ((debounce_val[0]==debounce_val[1])&&(debounce_val[1]==debounce_val[2])&&(debounce_val[2]==debounce_val[3])) {
				...
				if (debounce_ave >= 1) {
					SWITCH_DBG("h-5\n");
					/*将数据中断关闭，利用软中断，重新采集数据判断是否是三段耳机、四段耳机*/
					hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_DIRQ, 0x0);					/*23*/
					if (((&switch_data->timer) != NULL)) {
						del_timer(&switch_data->timer);
					}
...
static void codec_init_events(struct work_struct *work)
{
    ...
    if(special_earphone)	//?????????????,?????,????????????
    {
        ...
        tmp = hmic_rdreg(SUN6I_HMIC_DATA);
    	tmp1 =(tmp&0x1f);
    	switch_data1->mode = HEADPHONE_IDLE;
        SWITCH_DBG("%s,%d:tmp1 = %d\n",__func__,__LINE__,tmp1);
        if(tmp1 == 0)
        {
            switch_data1->state = 2;
    		headphone_state = 1;
            switch_data1->mode = THREE_HEADPHONE_PLUGIN;
    		down(&switch_data1->sem);
    		switch_set_state(&switch_data1->sdev, switch_data1->state);
    		up(&switch_data1->sem);
        }
    }
}
...
static int gpio_switch_probe(struct platform_device *pdev)
{
    ...
    type = script_get_item("audio_para", "special_earphone_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk("get special_earphone_used err,not used!\n");
	}
    else
    {
	    special_earphone = val.val;
        printk("%s,%d:special_earphone = %d\n",__func__,__LINE__,special_earphone);
    }

	hmic_base = (void __iomem *)VIR_CODEC_BASSADDRESS;
	hmic_wr_control(SUN6I_HMIC_CTL, 0xf, HMIC_M, 0x0);						/*0xf should be get from hw_debug 28*/
	hmic_wr_control(SUN6I_HMIC_CTL, 0xf, HMIC_N, 0x0);						/*0xf should be get from hw_debug 24 0xf*/
	hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_DIRQ, 0x1);					/*23*/
	...
	hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_KEY_UP_IRQ_EN, 0x1); 			/*18*/
	hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_KEY_DOWN_IRQ_EN, 0x1); 		/*17*/
	hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_DATA_IRQ_EN, 0x1); 			/*16*/
	hmic_wr_control(SUN6I_HMIC_CTL, 0x3, HMIC_DS_SAMP, 0x0); 				/*14 */
	hmic_wr_control(SUN6I_HMIC_CTL, 0x1f, HMIC_TH2_KEY, 0x0);				/*0xf should be get from hw_debug 8*/
...
static int switch_resume(struct platform_device *pdev)
{
	...
	if (check_scene_locked(SCENE_TALKING_STANDBY) != 0) {
		hmic_wr_control(SUN6I_HMIC_CTL, 0xf, HMIC_M, 0x0);						/*0xf should be get from hw_debug 28*/
		hmic_wr_control(SUN6I_HMIC_CTL, 0xf, HMIC_N, 0x0);						/*0xf should be get from hw_debug 24*/
		hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_EARPHONE_OUT_IRQ_EN, 0x1); 	/*20*/
		hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_EARPHONE_IN_IRQ_EN, 0x1); 	/*19*/
		hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_KEY_UP_IRQ_EN, 0x1); 			/*18*/
		hmic_wr_control(SUN6I_HMIC_CTL, 0x1, HMIC_KEY_DOWN_IRQ_EN, 0x1); 		/*17*/
