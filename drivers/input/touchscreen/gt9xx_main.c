...
void int2io(int status,int level)
{
    ...
    if(status == 1)
    {
        printk("irq pin set to output function\n");
...
//    u8 cfg_info_group1[] = CTP_CFG_GROUP1;
    ...
    /*
    u8 *send_cfg_buf[3] = {cfg_info_group2, gt9xx_fw_grp[fw_index].data, cfg_info_group3};
    u8 cfg_info_len[3] = {sizeof(cfg_info_group2)/sizeof(cfg_info_group2[0]), 
                          gt9xx_fw_grp[fw_index].size,
                          sizeof(cfg_info_group3)/sizeof(cfg_info_group3[0])};
    */
    //stone 20140122
    u8 *send_cfg_buf[3] = {cfg_info_group2, cfg_info_group3,  gt9xx_fw_grp[fw_index].data};
    u8 cfg_info_len[3] = {sizeof(cfg_info_group2)/sizeof(cfg_info_group2[0]),
                          sizeof(cfg_info_group3)/sizeof(cfg_info_group3[0]), 
                          gt9xx_fw_grp[fw_index].size};
    ...
    else
    {
        ...
        //stone 20140122
        if(rd_cfg_buf[GTP_ADDR_LENGTH] >= 0x03)
        {
        
          GTP_ERROR("Read SENSOR ID wrong,default use group1 config!");
        	rd_cfg_buf[GTP_ADDR_LENGTH] = 0;
        }
    }
...
static int __devinit goodix_ts_init(void)
{
    ...
    GTP_DEBUG_FUNC();	
    printk("gt9xx Dirver Complied Time:%s %s\n",__DATE__,__TIME__);
