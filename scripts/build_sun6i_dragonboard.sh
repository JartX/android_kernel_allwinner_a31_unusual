...
{	
	if [ "$BOOT_VERSION" = "boot_v1.0" ]; then
		echo "build nand library ${LICHEE_KDIR}/modules/nand/lib"
		NAND_ROOT=${LICHEE_KDIR}/modules/nand
	elif [ "$BOOT_VERSION" = "boot_v2.0" ]; then
		echo "build nand library ${LICHEE_KDIR}/modules/nand_v2/lib"
		NAND_ROOT=${LICHEE_KDIR}/modules/nand_v2
	else
	...
	if [ -d ${NAND_ROOT}/lib ]; then
			make -C ${NAND_ROOT}/lib clean 2>/dev/null
			make -C ${NAND_ROOT}/lib lib install
		else
...
copy_nand_mod()
{
    ...
    if [ -x "./scripts/build_rootfs.sh" ]; then
        ./scripts/build_rootfs.sh e rootfs_dragonboard.cpio.gz >/dev/null
    else
    ...
    if [ -f "./rootfs.cpio.gz" ]; then
        rm rootfs_dragonboard.cpio.gz
    fi
    ./scripts/build_rootfs.sh c rootfs_dragonboard.cpio.gz >/dev/null
    rm -rf skel
...
build_modules()
{
	...
	if [ "${BOOT_VERSION}" = "boot_v1.0" ]; then
		...
		make -C modules/nand_v2 LICHEE_MOD_DIR=${LICHEE_MOD_DIR} LICHEE_KDIR=${LICHEE_KDIR} \
			CONFIG_CHIP_ID=${CONFIG_CHIP_ID} install
	else
		echo -e "\033[0;31;1m################################################\033[0m"
		echo -e "\033[0;31;1m#        make sure boot version is correct     #\033[0m"
...
build_ramfs()
{
	cp rootfs_dragonboard.cpio.gz output/
	mkbootimg	--kernel output/bImage \
				--ramdisk output/rootfs_dragonboard.cpio.gz \
				--board 'a31' \
				--base 0x40000000 \
				-o output/boot.img
	rm  rootfs_dragonboard.cpio.gz
...
export PATH=${LICHEE_ROOT}/buildroot/output/external-toolchain/bin:${LICHEE_ROOT}/tools/${PACK_ROOT}/pctools/linux/android:$PATH
