...
#define NAND_LOG_RELEASE_LEVEL   32
...
static int nandlogrelease_thread(void *tmparg)
{
	...
	while (!arg->quit)
	{
		...
		printk("[NAND] after wakeup in log release thread\n");
		if(ret==0)
		{
		#if 0
			do{
...
static int nand_ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd, unsigned long arg)
{
	...
	case BLKBURNBOOT0:
		...
		if (0 == down_trylock(&(nandr->nand_ops_mutex)))
		{
			...
			up(&(nandr->nand_ops_mutex));
			dbg_err("do BLKBURNBOOT0!\n");
			IS_IDLE = 1;
			return ret;
		}
		else
		{
			dbg_err("not BLKBURNBOOT0!\n");
			return -ENOTTY;
		}
#else
		down(&nandr->nand_ops_mutex);
	...
	case BLKBURNBOOT1:
		...
		if (0 == down_trylock(&(nandr->nand_ops_mutex)))
		{
			...
			up(&(nandr->nand_ops_mutex));
			dbg_err("do BLKBURNBOOT1!\n");
			IS_IDLE = 1;
			return ret;
		}
		else
		{
			dbg_err("not BLKBURNBOOT1!\n");
			return -ENOTTY;
		}  
#else
