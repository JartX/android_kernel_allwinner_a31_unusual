...
unsigned int flush_cache_num = 8;
...
int _dev_nand_read(struct _nand_dev *nand_dev,__u32 start_sector,__u32 len,unsigned char *buf)
{
    ...
    //ret =  nand_dev->nftl_blk->read_sector_data(nand_dev->nftl_blk,start_sector + nand_dev->offset,len,buf);
    mutex_unlock(nftl_blk->blk_lock);
...
int _dev_nand_write(struct _nand_dev *nand_dev,__u32 start_sector,__u32 len,unsigned char *buf)
{
    ...
    //ret = nand_dev->nftl_blk->write_sector_data(nand_dev->nftl_blk,start_sector + nand_dev->offset,len,buf);
